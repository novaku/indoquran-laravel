...
function HomePage() {
    ...
    return (
        <>
            <PWAInstallPrompt />
            <PageTransition>
                <div className="min-h-screen bg-gradient-to-br from-islamic-green/5 to-islamic-gold/5 pb-16 sm:pb-20 lg:pb-24">
                    <SEOHead {...getHomeSEO()} />
                    ...
                    <div className="max-w-5xl mx-auto px-2 xs:px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12">
                        ...
                        {/* Widget Pencarian */}
                        <div className="mb-3 xs:mb-4 sm:mb-6 md:mb-8 mt-3 xs:mt-4 sm:mt-6 md:mt-8 w-full">
                            <SearchField 
                                surahs={surahs}
                                ...
                                theme="islamic"
                            />
                            <p className="text-center text-xs xs:text-sm sm:text-base text-islamic-green/70 mt-2 xs:mt-3 sm:mt-4 px-1 xs:px-2 flex items-center justify-center gap-1 xs:gap-1.5 sm:gap-2 max-w-2xl mx-auto">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 xs:h-4 xs:w-4 sm:h-5 sm:w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <RunningTextWidget className="mb-3 xs:mb-4 sm:mb-6 md:mb-8" />
                        
                        <PrayerTimesWidget className="mb-6 xs:mb-8 sm:mb-12 md:mb-16 lg:mb-20 p-1 xs:p-2 sm:p-3 md:p-4" />
                        
                        {/* Bagian Tab Fitur */}
                        <FeatureTabsSection className="mb-6 xs:mb-8 sm:mb-10 md:mb-12 lg:mb-16 p-1 xs:p-2 sm:p-3 md:p-4 mt-3 xs:mt-4 sm:mt-6 md:mt-8" />
                    
                        <div className="mb-3 xs:mb-4 sm:mb-6 md:mb-8">
                            <h1 className="text-lg xs:text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-islamic-green mb-1 xs:mb-2 leading-tight">Al-Quran Digital</h1>
                            <p className="text-xs xs:text-sm sm:text-base md:text-lg text-gray-600 mb-2 xs:mb-3 sm:mb-4 md:mb-6 leading-relaxed max-w-3xl">Baca, dengar, dan pelajari Al-Quran secara online dengan terjemahan bahasa Indonesia</p>
                        </div>
                    
                        {loading ? (
                            <div className="flex justify-center items-center h-32 xs:h-40 sm:h-48 md:h-64">
                                <div className="animate-spin rounded-full h-8 w-8 xs:h-10 xs:w-10 sm:h-12 sm:w-12 border-t-2 border-b-2 border-islamic-green"></div>
                            </div>
                        ) : error ? (
            ...
            {showModal && selectedSurah && (
                <div 
                    className="fixed inset-0 flex items-center justify-center p-1 xs:p-2 sm:p-4 bg-black/60 backdrop-blur-sm z-50"
                    onClick={closeModal}
                    style={{ zIndex: 9999 }}
                >
                    <div 
                        className="bg-white rounded-lg xs:rounded-xl sm:rounded-2xl lg:rounded-3xl p-0 max-w-5xl w-full max-h-[98vh] xs:max-h-[95vh] overflow-hidden shadow-2xl relative transform transition-all"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header with Gradient Background */}
                        <div className="relative bg-gradient-to-r from-islamic-green to-islamic-gold text-black p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8 pb-4 xs:pb-6 sm:pb-8 md:pb-12">
                            {/* Background Pattern */}
                            ...
                            <button 
                                onClick={closeModal}
                                className="absolute top-1 xs:top-2 sm:top-3 md:top-4 right-1 xs:right-2 sm:right-3 md:right-4 text-black/80 hover:text-black transition-colors p-1 xs:p-1.5 sm:p-2 rounded-full hover:bg-white/10"
                            >
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center mb-1 xs:mb-2 sm:mb-3 md:mb-4">
                                    <div className="w-8 xs:w-10 sm:w-12 md:w-16 h-8 xs:h-10 sm:h-12 md:h-16 bg-white/20 rounded-md xs:rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center text-black font-bold text-sm xs:text-base sm:text-lg md:text-xl mr-1.5 xs:mr-2 sm:mr-3 md:mr-4 backdrop-blur-sm">
                                        {selectedSurah?.number}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h2 className="text-base xs:text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mb-0.5 xs:mb-1 text-black truncate">
                                            {selectedSurah?.name_latin || 'Surah'}
                                        </h2>
                                        <p className="text-black/90 text-xs xs:text-sm sm:text-base md:text-lg truncate">
                                ...
                                {/* Arabic Name */}
                                <div className="text-center mb-1 xs:mb-2 sm:mb-3 md:mb-4">
                                    <h3 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-arabic leading-none mb-1 xs:mb-2 text-black">
                        ...
                        {/* Content Area */}
                        <div className="p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8 max-h-[40vh] xs:max-h-[50vh] sm:max-h-[60vh] overflow-y-auto">
                            {/* Quick Stats */}
                            <div className="mb-4 xs:mb-6 sm:mb-8">
                                <div className="bg-gradient-to-r from-blue-50 to-amber-50 p-2 xs:p-3 sm:p-4 md:p-5 rounded-md xs:rounded-lg sm:rounded-xl border border-islamic-green/10">
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 xs:gap-3 sm:gap-4 md:gap-6">
                                        <div className="flex flex-col items-center text-blue-700">
                                            <div className="w-6 xs:w-8 sm:w-10 h-6 xs:h-8 sm:h-10 rounded-full bg-blue-100 flex items-center justify-center mb-1 xs:mb-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                                </svg>
                                            </div>
                                            <div className="text-center">
                                                <div className="text-xs xs:text-sm font-medium text-blue-600">Wahyu</div>
                                                <div className="font-bold text-blue-800 text-xs xs:text-sm">
                                                    {selectedSurah?.revelation_place}
                                                </div>
                                            </div>
                                        </div>                        
                        <div className="flex flex-col items-center text-purple-700">
                            <div className="w-6 xs:w-8 sm:w-10 h-6 xs:h-8 sm:h-10 rounded-full bg-purple-100 flex items-center justify-center mb-1 xs:mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-1l-4 4z" />
                                </svg>
                            </div>
                            <div className="text-center">
                                <div className="text-xs xs:text-sm font-medium text-purple-600">Ayat</div>
                                <div className="font-bold text-purple-800 text-xs xs:text-sm">
                        ...
                        <div className="flex flex-col items-center text-amber-700">
                            <div className="w-6 xs:w-8 sm:w-10 h-6 xs:h-8 sm:h-10 rounded-full bg-amber-100 flex items-center justify-center mb-1 xs:mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                                </svg>
                            </div>
                            <div className="text-center">
                                <div className="text-xs xs:text-sm font-medium text-amber-600">Urutan</div>
                                <div className="font-bold text-amber-800 text-xs xs:text-sm">
                        ...
                        <div className="flex flex-col items-center text-green-700">
                            <div className="w-6 xs:w-8 sm:w-10 h-6 xs:h-8 sm:h-10 rounded-full bg-green-100 flex items-center justify-center mb-1 xs:mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <div className="text-center">
                                <div className="text-xs xs:text-sm font-medium text-green-600">Periode</div>
                                <div className="font-bold text-green-800 text-xs xs:text-sm">
                                    {selectedSurah?.revelation_place}
                                </div>
                            </div>
                        </div>
                            ...
                            {selectedSurah?.description_short && (
                                <div className="mb-4 xs:mb-6 sm:mb-8">
                                    <h4 className="text-base xs:text-lg sm:text-xl font-bold text-gray-800 mb-2 xs:mb-3 sm:mb-4 flex items-center">
                                        <div className="w-5 xs:w-6 sm:w-8 h-5 xs:h-6 sm:h-8 bg-islamic-green/10 rounded-md xs:rounded-lg flex items-center justify-center mr-1.5 xs:mr-2 sm:mr-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5 text-islamic-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                                            </svg>
                                        </div>
                                        <span className="text-sm xs:text-base sm:text-lg md:text-xl">Tema & Kandungan Utama</span>
                                    </h4>
                                    <div className="bg-gradient-to-r from-islamic-green/5 to-islamic-gold/5 p-3 xs:p-4 sm:p-6 rounded-lg xs:rounded-xl sm:rounded-2xl border border-islamic-green/10">
                                        <div 
                                            className="text-gray-700 leading-relaxed prose prose-gray max-w-none text-xs xs:text-sm sm:text-base"
                            ...
                            {selectedSurah?.description_long && (
                                <div className="mb-4 xs:mb-6 sm:mb-8">
                                    <h4 className="text-base xs:text-lg sm:text-xl font-bold text-gray-800 mb-2 xs:mb-3 sm:mb-4 flex items-center">
                                        <div className="w-5 xs:w-6 sm:w-8 h-5 xs:h-6 sm:h-8 bg-islamic-gold/10 rounded-md xs:rounded-lg flex items-center justify-center mr-1.5 xs:mr-2 sm:mr-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5 text-islamic-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                            </svg>
                                        </div>
                                        <span className="text-sm xs:text-base sm:text-lg md:text-xl">Penjelasan Lengkap</span>
                                    </h4>
                                    <div className="bg-gradient-to-r from-amber-50/50 to-orange-50/50 p-3 xs:p-4 sm:p-6 rounded-lg xs:rounded-xl sm:rounded-2xl border border-amber-100">
                                        <div 
                                            className="text-gray-700 leading-relaxed prose prose-gray max-w-none text-xs xs:text-sm sm:text-base"
                            ...
                            {/* Fun Facts */}
                            <div className="mb-4 xs:mb-6 sm:mb-8 p-3 xs:p-4 sm:p-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg xs:rounded-xl sm:rounded-2xl border border-indigo-100">
                                <h4 className="text-sm xs:text-base sm:text-lg font-bold text-indigo-800 mb-2 xs:mb-3 sm:mb-4 flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 w-3 xs:w-4 sm:w-5 mr-1.5 xs:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    Fakta Menarik
                                </h4>
                                <div className="grid grid-cols-1 gap-1.5 xs:gap-2 sm:gap-4 text-xs xs:text-sm">
                                    <div className="flex items-start text-indigo-700">
                                        <span className="w-1 xs:w-1.5 sm:w-2 h-1 xs:h-1.5 sm:h-2 bg-indigo-400 rounded-full mr-1.5 xs:mr-2 sm:mr-3 mt-1.5 xs:mt-2 flex-shrink-0"></span>
                        ...
                        {/* Action Buttons */}
                        <div className="p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8 border-t border-gray-100 bg-gradient-to-r from-gray-50/50 to-gray-50/80">
                            <div className="space-y-1.5 xs:space-y-2 sm:space-y-3 md:space-y-4">
                                ...
                                <button
                                    ...
                                    className="w-full bg-gradient-to-r from-islamic-green to-emerald-600 hover:from-islamic-green/90 hover:to-emerald-600/90 text-white py-2 xs:py-2.5 sm:py-3 md:py-4 px-2 xs:px-3 sm:px-4 md:px-6 rounded-md xs:rounded-lg sm:rounded-xl font-semibold text-xs xs:text-sm sm:text-base md:text-lg flex items-center justify-center gap-1 xs:gap-1.5 sm:gap-2 md:gap-3 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 active:scale-98 group"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 xs:h-4 sm:h-5 md:h-6 w-3 xs:w-4 sm:w-5 md:w-6 group-hover:scale-110 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                    </svg>
                                ...
                                <div className="grid grid-cols-2 gap-1.5 xs:gap-2 sm:gap-3">
                                    <button
                                        ...
                                        className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white py-1.5 xs:py-2 sm:py-2.5 md:py-3 px-1.5 xs:px-2 sm:px-3 md:px-4 rounded-md xs:rounded-lg font-medium text-xs xs:text-sm md:text-base flex items-center justify-center gap-0.5 xs:gap-1 sm:gap-1.5 md:gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 active:scale-98 group"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 xs:h-3 sm:h-4 md:h-5 w-2.5 xs:w-3 sm:w-4 md:w-5 group-hover:scale-110 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    ...
                                    <button
                                        onClick={closeModal}
                                        className="bg-white hover:bg-gray-50 border-2 border-gray-300 hover:border-gray-400 text-gray-700 hover:text-gray-800 py-1.5 xs:py-2 sm:py-2.5 md:py-3 px-1.5 xs:px-2 sm:px-3 md:px-4 rounded-md xs:rounded-lg font-medium text-xs xs:text-sm md:text-base flex items-center justify-center gap-0.5 xs:gap-1 sm:gap-1.5 md:gap-2 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 active:scale-98 group"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 xs:h-3 sm:h-4 md:h-5 w-2.5 xs:w-3 sm:w-4 md:w-5 group-hover:scale-110 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
