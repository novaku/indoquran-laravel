...
log_message "Checking Composer availability..."
if command -v composer >/dev/null 2>&1; then
    log_message "✓ Composer found: $(composer --version | head -n1)"
else
    log_error "Composer not found!"
    log_error "Please install Composer first:"
...
EOF
    ...
    # Test Redis connection using the PHP script (only if vendor exists)
    if [ -d "vendor" ]; then
        ...
        if [[ "$redis_test_result" == SUCCESS:* ]]; then
            ...
            log_warning "⚠ Redis server is not responding - will test again after Composer install"
            redis_working=false
        fi
    else
        log_message "Vendor directory not found, will test Redis after Composer install"
        rm -f test_redis_connection.php
        redis_working=false
    fi
...
if [ -f "composer.lock" ]; then
    ...
    composer install --no-dev --optimize-autoloader --no-interaction || { log_error "Composer install failed"; exit 1; }
else
...
# Re-test Redis connection after Composer install (if it wasn't working before)
if [ "$redis_working" = false ] && grep -q "CACHE_STORE=redis" .env; then
    log_message "Re-testing Redis connection after Composer install..."
